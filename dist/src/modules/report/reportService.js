...
class ReportService {
    ...
    async getReport(data) {
        ...
        if (report) {
            ...
            return responseBuilder_1.ResponseBuilder.data(reportData, l10n.t("REPORT_GET"));
        }
        else {
            return responseBuilder_1.ResponseBuilder.errorMessage(l10n.t("ERR_INTERNAL_SERVER"));
        }
    ...
    async getAllReport(data) {
        try {
            ...
            if (report?.length) {
                // Add flag to indicate logos should not be added to individual images in PDF
                // Logo should only appear at the top right of the PDF, not on each image
                const reportWithFlag = report.map((r) => {
                    const reportItem = { ...r };
                    reportItem.addLogoToImages = false;
                    return reportItem;
                });
                return responseBuilder_1.ResponseBuilder.data(reportWithFlag, l10n.t("REPORT_GET"));
            }
            else {
