...
class TraineeService {
    ...
    async bookSession(payload, _id) {
        try {
            ...
            const subjectTrainee = `NetQwix Training Session Booked for ${bookedDate} at ${bookedTime} ${constant_1.timeZoneAbbreviations[sessionObj.time_zone] || sessionObj.time_zone}`;
            const timeZoneInShortForTrainer = luxon_1.DateTime.now()
                .setZone(trainerDetails.extraInfo.availabilityInfo.timeZone)
                .toFormat("ZZZZ");
            const subjectTrainer = `NetQwix Training Session Booked for ${bookedDate} at ${bookedTime} ${constant_1.timeZoneAbbreviations[trainerDetails.extraInfo.availabilityInfo.timeZone] || trainerDetails.extraInfo.availabilityInfo.timeZone}`;
            ...
            if (traineeDetails.notifications.transactional.email) {
                sendEmail_1.SendEmail.sendRawEmail("session-booking-trainee", {
                    "[TRAINEE FIRST NAME]": traineeDetails.fullname.split(" ")[0],
                    "[TRAINER NAME]": trainerDetails.fullname,
                    "[session date and time]": `${bookedDate} at ${bookedTime} ${constant_1.timeZoneAbbreviations[sessionObj.time_zone] || sessionObj.time_zone}`,
                    "[MEETING_LINK]": meetingLink
                }, traineeDetails.email, "NetQwix Training Session is Booked");
            }
            if (trainerDetails.notifications.transactional.email) {
                sendEmail_1.SendEmail.sendRawEmail("session-booking-trainer", {
                    "[TRAINER FIRST NAME]": trainerDetails.fullname.split(" ")[0],
                    "[TRAINEE_NAME]": traineeDetails.fullname,
                    "[session date and time]": `${bookedDate} at ${bookedTime} ${constant_1.timeZoneAbbreviations[trainerDetails.extraInfo.availabilityInfo.timeZone] || trainerDetails.extraInfo.availabilityInfo.timeZone}`,
                    "[MEETING_LINK]": meetingLink
                }, trainerDetails.email, "NetQwix Training Session is Booked");
            }
            ...
            if (payload.status === constance_1.BOOKED_SESSIONS_STATUS["BOOKED"]) {
                if (traineeDetails.notifications.transactional.email) {
                    sendEmail_1.SendEmail.sendRawEmail("payment-confirmation", {
                        "[First Name]": traineeDetails.fullname.split(" ")[0],
                        "[AMOUNT]": charging_price,
                        "[TRAINER NAME]": trainerDetails.fullname,
                        "[TRAINER NAME2]": trainerDetails.fullname
                    }, [traineeDetails.email], "NetQwix Payment Confirmation");
                }
            }
    ...
    async bookInstantMeeting(payload, _id) {
        ...
        try {
            ...
            const duration = durationMinutes && [15, 30, 60, 120].includes(Number(durationMinutes))
                ...
                : 30;
            const session_start_time = dateFormat_1.DateFormat.addMinutes(booked_date, 0, constance_1.CONSTANCE.INSTANT_MEETING_TIME_FORMAT);
            const session_end_time = dateFormat_1.DateFormat.addMinutes(booked_date, duration, constance_1.CONSTANCE.INSTANT_MEETING_TIME_FORMAT);
            ...
            return responseBuilder_1.ResponseBuilder.data({ bookingId: bookingData._id, booking: bookingData }, l10n.t("INSTANT_MEETING_BOOKED"));
        }
        catch (err) {
            return responseBuilder_1.ResponseBuilder.error(err, l10n.t("ERR_INTERNAL_SERVER"));
        }
