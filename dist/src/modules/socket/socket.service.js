...
exports.emitBookingStatusUpdated = exports.emitBookingCreated = exports.handleSocketEvents = exports.setIoInstance = void 0;
...
let activeUsers = {};
let ioInstance = null; // Store io instance for emitting events from services
...
const emitLessonStateSync = (socket, roomName, session) => {
    const statePayload = {
    ...
    if (ioInstance)
        ...
        socket.nsp.to(roomName).emit("LESSON_STATE_SYNC", statePayload);
};
const clearLessonTimeouts = (session) => {
...
const scheduleLessonEnd = (socket, roomName, session) => {
    ...
    if (remainingMs <= 0) {
        ...
        if (ioInstance)
            ioInstance.to(roomName).emit(constance_1.EVENTS.LESSON_TIMER.ENDED, endedPayload);
        else
            socket.nsp.to(roomName).emit(constance_1.EVENTS.LESSON_TIMER.ENDED, endedPayload);
    ...
    try {
        ...
        if (socket?.user?._doc?.account_type === "Trainer") {
            ...
            if (socket.user._doc._id) {
                const checkIfUserIsAlreadyAdded = await online_user_schema_1.default.findOne({
                ...
                if (checkIfUserIsAlreadyAdded) {
                    await online_user_schema_1.default.updateOne({ trainer_id: socket.user._doc._id }, { $set: { last_activity_time: Date.now() } });
                }
                else {
                    const createNewOnlineUser = await new online_user_schema_1.default({
                        trainer_id: socket.user._doc._id,
...
const handleSocketEvents = (socket, connections = {}) => {
        ...
        socket.rooms.forEach((roomName) => {
            if (roomName.startsWith("session:")) {
                ...
                if (session) {
                    ...
                    socket.to(roomName).emit(constance_1.EVENTS.VIDEO_CALL.ON_CALL_LEAVE, {
                        userId,
                        accountType,
                        sessionId,
                        timestamp: Date.now()
    ...
    socket.on(constance_1.EVENTS.JOIN_ROOM, async (socketReq, request) => {
    ...
    socket.on("CLIENT_CALL_DIAGNOSTICS", async (payload) => {
        try {
            ...
            if (sessionId && userId) {
                try {
                    await call_diagnostics_schema_1.default.create({
                        sessionId,
                        userId,
                        accountType,
                        role,
            ...
            console.error("[CallDiagnostics] Failed to process CLIENT_CALL_DIAGNOSTICS:", err);
        }
    });
    ...
    socket.on("CLIENT_PRECALL_CHECK", async (payload) => {
        try {
            ...
            console.error("[PreCallCheck] Failed to process CLIENT_PRECALL_CHECK:", err);
        }
    });
    ...
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_OFFER, ({ offer, userInfo }) => {
        const toUserId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserId).emit("offer", offer);
        // TODO:for now broadcasting the event, it needs to send to specific user.
        // socket.broadcast.emit('offer', offer);
    });
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_CALL_JOIN, async ({ userInfo }) => {
        const toUserId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        ...
        const accountType = socket?.user?._doc?.account_type || socket?.user?.account_type;
        const userId = socket?.user?._doc?._id || socket?.user?._id;
        if (sessionId && mongoose_1.default.isValidObjectId(sessionId)) {
            ...
            if (!session) {
                ...
                try {
                    const bookedSession = await booked_sessions_schema_1.default.findById(sessionId);
                    if (bookedSession) {
            ...
            if (session) {
                ...
                const otherSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
                if (otherSocketId) {
                    ...
                    if (otherSocket) {
                        ...
                        otherSocket.emit(constance_1.EVENTS.VIDEO_CALL.ON_CALL_JOIN, {
                            userInfo: {
                                ...userInfo,
                                joinedUserId: userId,
                ...
                if (session.status === "running" && session.startedAt != null) {
                    ...
                    socket.emit(constance_1.EVENTS.LESSON_TIMER.STARTED, timerPayload);
        ...
        if (toUserId) {
            socket.to(toUserId).emit(constance_1.EVENTS.VIDEO_CALL.ON_CALL_JOIN, { userInfo });
        }
    });
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_BOTH_JOIN, async (socketReq) => {
        const toUserId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, socketReq.userInfo?.to_user);
        ...
        socket.to(toUserId).emit(constance_1.EVENTS.VIDEO_CALL.ON_BOTH_JOIN, { socketReq });
    });
    ...
    socket.on("LESSON_TIMER_START_REQUEST", ({ sessionId }) => {
        ...
        if (ioInstance)
            ioInstance.to(roomName).emit(constance_1.EVENTS.LESSON_TIMER.STARTED, timerPayload);
        else
            socket.nsp.to(roomName).emit(constance_1.EVENTS.LESSON_TIMER.STARTED, timerPayload);
        emitLessonStateSync(socket, roomName, session);
    ...
    socket.on("LESSON_TIMER_PAUSE_REQUEST", ({ sessionId }) => {
        ...
        if (ioInstance)
            ioInstance.to(roomName).emit("LESSON_TIME_PAUSED", pausedPayload);
        else
            socket.nsp.to(roomName).emit("LESSON_TIME_PAUSED", pausedPayload);
        emitLessonStateSync(socket, roomName, session);
    });
    socket.on("LESSON_TIMER_RESUME_REQUEST", ({ sessionId }) => {
        ...
        if (ioInstance)
            ioInstance.to(roomName).emit("LESSON_TIME_RESUMED", resumedPayload);
        else
            socket.nsp.to(roomName).emit("LESSON_TIME_RESUMED", resumedPayload);
        emitLessonStateSync(socket, roomName, session);
        scheduleLessonEnd(socket, roomName, session);
    });
    ...
    // });
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_ICE_CANDIDATE, (data) => {
        const { userInfo } = data;
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        // Broadcast the ICE candidate to the other connected peers
        socket.to(toUserSocketId).emit("ice-candidate", data);
    });
    socket.on(constance_1.EVENTS.EMIT_CLEAR_CANVAS, (payload) => {
        const { userInfo } = payload;
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_CLEAR_CANVAS, {});
    });
    socket.on(constance_1.EVENTS.EMIT_UNDO, (payload) => {
        const { userInfo } = payload;
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_UNDO, payload);
    });
    socket.on(constance_1.EVENTS.VIDEO_CALL.MUTE_ME, ({ muteStatus, userInfo }) => {
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserSocketId).emit(constance_1.EVENTS.VIDEO_CALL.MUTE_ME, { muteStatus });
    });
    socket.on(constance_1.EVENTS.VIDEO_CALL.STOP_FEED, ({ feedStatus, userInfo }) => {
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserSocketId).emit(constance_1.EVENTS.VIDEO_CALL.STOP_FEED, { feedStatus });
    });
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_CLOSE, (payload) => {
        const { userInfo } = payload;
        const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
        socket.to(toUserSocketId).emit(constance_1.EVENTS.VIDEO_CALL.ON_CLOSE, {});
    });
    // Listen for userActivity event
    // socket.on('userActivity', (data) => {
    updateUserActivity(socket);
    // });
    const handleRoomJoinEvent = (roomName) => {
...
const listenNotificationEvents = (socket) => {
    try {
        socket.on(constance_1.EVENTS.PUSH_NOTIFICATIONS.ON_SEND, async (payload) => {
            ...
            const newNotifications = await notifications_schema_1.default.create({
                title,
                description,
                senderId,
                receiverId,
                type: type ?? notification_enum_1.NotificationType.DEFAULT
            });
            ...
            socket.to(toUserSocketId).emit(constance_1.EVENTS.PUSH_NOTIFICATIONS.ON_RECEIVE, {
...
const listenInstantLessonEvents = (socket) => {
    try {
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.REQUEST, async (payload) => {
            try {
                ...
                const coachSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, coachId);
                if (coachSocketId) {
                    socket.to(coachSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.REQUEST, {
                        lessonId,
                        coachId,
                        traineeId,
                        traineeInfo,
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.ACCEPT, async (payload) => {
            try {
                ...
                const coachSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, coachId);
                const traineeSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, traineeId);
                if (coachSocketId) {
                    socket.to(coachSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.ACCEPT, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
                ...
                if (traineeSocketId) {
                    socket.to(traineeSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.ACCEPT, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.DECLINE, async (payload) => {
            try {
                const { lessonId, coachId, traineeId } = payload;
                if (!lessonId || !coachId || !traineeId) {
                ...
                const traineeSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, traineeId);
                if (traineeSocketId) {
                    socket.to(traineeSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.DECLINE, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.EXPIRE, async (payload) => {
            try {
                const { lessonId, coachId, traineeId } = payload;
                if (!lessonId) {
                ...
                const coachSocketId = coachId ? memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, coachId) : null;
                const traineeSocketId = traineeId ? memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, traineeId) : null;
                if (coachSocketId) {
                    socket.to(coachSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.EXPIRE, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
                ...
                if (traineeSocketId) {
                    socket.to(traineeSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.EXPIRE, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.CLIPS_SELECTED, async (payload) => {
            try {
                const { lessonId, coachId, traineeId } = payload;
                if (!lessonId || !coachId) {
                ...
                const coachSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, coachId);
                if (coachSocketId) {
                    socket.to(coachSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.CLIPS_SELECTED, {
                        lessonId,
                        coachId,
                        traineeId,
                    });
        ...
        socket.on(constance_1.EVENTS.INSTANT_LESSON.TRAINEE_CANCELLED, async (payload) => {
            try {
                const { lessonId, coachId, traineeId } = payload;
                if (!lessonId || !coachId)
                    return;
                const coachSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, coachId);
                if (coachSocketId) {
                    socket.to(coachSocketId).emit(constance_1.EVENTS.INSTANT_LESSON.TRAINEE_CANCELLED, { lessonId, coachId, traineeId });
...
const listenBookingEvents = (socket) => {
    try {
        ...
        socket.on(constance_1.EVENTS.BOOKING.CREATED, async (payload) => {
            console.log(`[BOOKING] Booking created event received:`, payload);
        });
        socket.on(constance_1.EVENTS.BOOKING.STATUS_UPDATED, async (payload) => {
...
const emitBookingCreated = async (bookingData, bookingType = 'scheduled') => {
    try {
        if (!ioInstance) {
        ...
        const startTimeUtc = bookingData?.start_time ? new Date(bookingData.start_time).toISOString() : null;
        const endTimeUtc = bookingData?.end_time ? new Date(bookingData.end_time).toISOString() : null;
        const bookedDateUtc = bookingData?.booked_date ? new Date(bookingData.booked_date).toISOString() : null;
        ...
        // Emit to trainer
        const trainerSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, trainerId);
        if (trainerSocketId && ioInstance) {
            ioInstance.to(trainerSocketId).emit(constance_1.EVENTS.BOOKING.CREATED, payload);
            console.log(`[BOOKING] BOOKING_CREATED event emitted to trainer ${trainerId}`);
        }
        // Emit to trainee
        const traineeSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, traineeId);
        if (traineeSocketId && ioInstance) {
            ioInstance.to(traineeSocketId).emit(constance_1.EVENTS.BOOKING.CREATED, payload);
...
const emitBookingStatusUpdated = async (bookingData) => {
    try {
        if (!ioInstance) {
        ...
        const trainerSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, trainerId);
        if (trainerSocketId && ioInstance) {
            ioInstance.to(trainerSocketId).emit(constance_1.EVENTS.BOOKING.STATUS_UPDATED, payload);
            console.log(`[BOOKING] BOOKING_STATUS_UPDATED event emitted to trainer ${trainerId}`);
        }
        // Emit to trainee
        const traineeSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, traineeId);
        if (traineeSocketId && ioInstance) {
            ioInstance.to(traineeSocketId).emit(constance_1.EVENTS.BOOKING.STATUS_UPDATED, payload);
...
exports.emitBookingStatusUpdated = emitBookingStatusUpdated;
const listenDrawEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.DRAW, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            // Broadcast the offer to the other connected peers
            // console.log(`toUserSocketId --- `, toUserSocketId);
            // console.log(`socket req ==== `, socketReq, socket.id)
            socket.to(toUserSocketId).emit(constance_1.EVENTS.EMIT_DRAWING_CORDS, socketReq);
        });
    }
    catch (err) {
...
const stopDrawEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.STOP_DRAWING, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.EMIT_STOP_DRAWING, socketReq);
        });
    }
    catch (err) {
...
const listenVideoShowEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.ON_VIDEO_SHOW, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_VIDEO_SHOW, socketReq);
        });
    }
    catch (err) {
...
const listenDrawingModeToggle = (socket) => {
    try {
        socket.on(constance_1.EVENTS.TOGGLE_DRAWING_MODE, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.TOGGLE_DRAWING_MODE, socketReq);
        });
    }
    catch (err) {
...
const listenFullscreenToggle = (socket) => {
    try {
        socket.on(constance_1.EVENTS.TOGGLE_FULL_SCREEN, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.TOGGLE_FULL_SCREEN, socketReq);
        });
    }
    catch (err) {
...
const listenLockModeToggle = (socket) => {
    try {
        socket.on(constance_1.EVENTS.TOGGLE_LOCK_MODE, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.TOGGLE_LOCK_MODE, socketReq);
        });
    }
    catch (err) {
...
const listenVideoPositionEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.ON_VIDEO_ZOOM_PAN, async (socketReq, request) => {
            const { userInfo, sessionId } = socketReq;
            // Prefer session room so trainee receives zoom/pan even after reconnect
            if (sessionId && mongoose_1.default.isValidObjectId(sessionId)) {
                const roomName = `session:${sessionId}`;
                socket.to(roomName).emit(constance_1.EVENTS.ON_VIDEO_ZOOM_PAN, socketReq);
            }
            else {
                const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
                if (toUserSocketId) {
                    socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_VIDEO_ZOOM_PAN, socketReq);
                }
            }
        });
    }
...
const listenShowVideoEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.ON_VIDEO_SELECT, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_VIDEO_SELECT, socketReq);
        });
    }
    catch (err) {
...
const listenCallEndEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.CALL_END, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.CALL_END, socketReq);
        });
    }
    catch (err) {
...
const listenPlayPauseVideoEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.ON_VIDEO_PLAY_PAUSE, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_VIDEO_PLAY_PAUSE, socketReq);
        });
    }
    catch (err) {
...
const listenVideoTimeEvent = (socket) => {
    try {
        socket.on(constance_1.EVENTS.ON_VIDEO_TIME, async (socketReq, request) => {
            const { userInfo } = socketReq;
            const toUserSocketId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
            socket.to(toUserSocketId).emit(constance_1.EVENTS.ON_VIDEO_TIME, socketReq);
        });
    }
    catch (err) {
