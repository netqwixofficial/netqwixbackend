    ...
    if (socket?.user?._doc?.account_type === "Trainer") {
        ...
        if (socket.user._doc._id) {
            ...
            if (checkIfUserIsAlreadyAdded) {
                ...
                const createNewOnlineUser = new online_user_schema_1.default({
                    trainer_id: socket.user._doc._id,
...
const handleSocketEvents = (socket, connections = {}) => {
    ...
    socket.on(constance_1.EVENTS.VIDEO_CALL.ON_CALL_JOIN, ({ userInfo }) => {
        const toUserId = memCache_1.MemCache.getDetail(process.env.SOCKET_CONFIG, userInfo?.to_user);
...
const listenVideoChunksEvent = (socket) => {
                ...
                if (code !== 0) {
                    ...
                    generatePreSignedPutUrl(fileName, "video/mp4").then(async (url) => {
                        await axios
                            ...
                            .then(async (response) => {
                            const savedSessionObj = new saved_sessions_schema_1.default(payload);
    ...
    socket.on(constance_1.EVENTS.ON_DISCONNECT, () => {
