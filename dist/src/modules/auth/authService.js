    ...
    constructor() {
        ...
        this.createNewUser = async (createUser) => {
            ...
            const userObj = referredUser
                ? new user_schema_1.default({ ...updateduserObj, _id: referredUser._id, friends: [referredUser.referrerId] }) // Use referred user's _id
            ...
            if (referredUser) {
                const rUser = await user_schema_1.default.findById(referredUser.referrerId);
            ...
            if (createUser.account_type === authEnum_1.AccountType.TRAINER) {
                sendEmail_1.SendEmail.sendRawEmail("new-trainer", {
                ...
                }, [adminEmail], `NetQwix New Expert Sign Up Request from ${createUser.fullname}`);
            }
            else {
                await user_schema_1.default.findByIdAndUpdate(userObj._id, { status: "approved" }, { new: true });
                sendEmail_1.SendEmail.sendRawEmail("new-trainee", {
                    "[TRAINER_NAME]": createUser.fullname,
                    "[TRAINER_NAME2]": createUser.fullname,
                    "[EMAIL_AND_NUMBER]": `${createUser.email}, ${createUser.mobile_no}.`
                }, [adminEmail], `NetQwix New Enthusiast - ${createUser.fullname}`);
            }
